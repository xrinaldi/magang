...
function setupScrollAnimations() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Jika link mengandung hash dan elemen target ada di halaman
            if (targetId.includes('#')) {
                let targetElement;
                
                // Periksa apakah link mengarah ke halaman lain
                if (this.hostname !== window.location.hostname || this.pathname !== window.location.pathname) {
                    // Biarkan browser menangani navigasi normal ke halaman lain
                    return;
                }
                
                // Jika hash dengan "#" saja (target di halaman saat ini)
...
function setupBlogInteractions() {
    // Mengecek apakah ada tombol "Lihat Semua Artikel" dan menambahkan event listener
    ...
    // Memastikan link blog bernavigasi dengan benar
    // Tidak perlu custom handler karena link blog sudah menggunakan route yang benar
    // Hanya pastikan link blog memiliki href yang sesuai dari controller
}