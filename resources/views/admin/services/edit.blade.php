        ...
        <form action="{{ route('admin.services.update', $service->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <div id="features-container">
                    @php
                        $features = old('features', is_array($service->features) ? $service->features : json_decode($service->features, true) ?? []);
                    @endphp
            ...
            <div class="mb-3">
                ...
                <div id="packages-container" class="mb-3">
                    <!-- Packages will be added here by JavaScript -->
                </div>
...
<script>
    ...
    function addPackage(packageData = null) {
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" name="packages[${packageId}][name]" value="${packageData?.name || ''}" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Harga (Rp)</label>
                        <input type="number" class="form-control" name="packages[${packageId}][price]" value="${packageData?.price || ''}" required>
                    </div>
                </div>
                
                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" id="popularPackage${packageId}" name="packages[${packageId}][popular]" ${packageData?.popular ? 'checked' : ''}>
                ...
                <div class="mb-3">
                    ...
                    <div class="package-features-container" id="package-features-${packageId}">
                        ${generatePackageFeatures(packageId, packageData?.features || [])}
                    </div>
    ...
    function generatePackageFeatures(packageId, features) {
        ...
        return features.map(feature => `
            <div class="package-feature-input-group">
                <div class="input-group mb-2">
                    <input type="text" class="form-control" name="packages[${packageId}][features][]" value="${feature}" required>
                    <button type="button" class="btn btn-danger" onclick="removePackageFeature(this)">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const servicePackages = @json(old('packages', is_array($service->packages) ? $service->packages : json_decode($service->packages, true) ?? []));
        
        if (servicePackages && servicePackages.length > 0) {
            servicePackages.forEach(packageData => {
                addPackage(packageData);
            });
        } else {
            // Add at least one empty package if none exists
            addPackage();
        }
    });
</script>
